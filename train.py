...
def train(opt, logger, opt_pl):
    ...
    for i in range(opt.train_iters): ### 60000
        ...
        if opt.stage == 'warm_up':
            ...
            loss_seg, loss, loss_CTS, loss_kd, loss_CTS_transfer, loss_ensemble_all = model.step(source_data, data_target_list, device)
        ...
        if (i + 1) % opt.print_interval == 0:
            if opt.stage == 'warm_up':
                ...
                fmt_str = "Iter [{:d}/{:d}] loss_seg: {:.4f} loss: {:.4f}  loss_CTS: {:.4f}  loss_kd: {:.4f}  " \
                          "loss_CTS_transfer: {:.4f} loss_ensemble_all: {:.4f} Time/Image: {:.4f}"
                print_str = fmt_str.format(i + 1, opt.train_iters, loss_seg, loss, loss_CTS,
                                           loss_kd, loss_CTS_transfer, loss_ensemble_all, time_meter.avg / opt.bs)
            print(print_str)
...
def validation(model, logger, datasets, device, running_metrics_val_list, iters, opt=None):
    ...
    for i_target in range(len(opt.tgt_dataset_list)):
        ...
        with torch.no_grad():
            if opt.stage == 'warm_up':
                validate(val_datset, device, model, running_metrics_val, len(opt.tgt_dataset_list)-1)
            else:
    ...
    for i in range(num_target):
        ...
        for data_i in tqdm(valid_loader_list[i]):
            ...
            if opt.name == 'mtaf_test_labv2_stage2':
                out = model.BaseNet_DP(images_val, [0], ssl=[])[0]
            else:
                out = model.BaseNet_DP(images_val, [i], ssl=[])[0]
            if opt.soft:
